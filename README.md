...
In this task, weights of the binary classification are obtained from meta-model which is specified as 2 layer Multilayer Perceptron. Since task is a linear classification, therefore, weights are directly multiplied with images vector in the query. Query examples contain both positive examples and negative examples. For example, from the figure, we can see that digit of 7 was selected as target class and meta model takes one image example of this class. Meta model produces weight matrix according to sample image. By multiplying weight matrix and vector of images in the query, probability of whether query images are belong to target class were computed.
...
# Results
Training loss and test accuracy values are recorded through training. At the end of the training, recorded values are visualize by using matplotlib library. Results were obtained for both MLP and CNN based models.
<p align="center">
...
--lr 3e-4 --output 'output/' --device 0
```
